...
const EMOTION_GENRE_MAP = {
  ...
  disgust: { movie: [27, 53], tv: [9648, 10765] },
...
export async function fetchHomeContent() {
  const trendingParams = { sort_by: 'popularity.desc', 'vote_count.gte': 500, 'vote_average.gte': 6 }
  const featuredParams = { sort_by: 'vote_average.desc', 'vote_count.gte': 1500, 'vote_average.gte': 6 }
  const popularSeriesParams = { sort_by: 'popularity.desc', 'vote_count.gte': 500, 'vote_average.gte': 6 }

  const [trendingMovies, trendingSeries, featuredMovies, popularSeries] = await Promise.all([
    fetchFromDiscover('movie', trendingParams, 15),
    fetchFromDiscover('tv', trendingParams, 15),
    fetchFromDiscover('movie', featuredParams, 15),
    fetchFromDiscover('tv', popularSeriesParams, 15),
  ])
...
export async function searchTmdb(searchParams) {
  ...
  if (query) {
    ...
    return {
      movies: (data?.results ?? [])
        .filter((item) => item.media_type === 'movie' && (item.vote_average || 0) >= 6)
        .map((item) => formatContent(item, 'movie'))
        .filter(Boolean),
      series: (data?.results ?? [])
        .filter((item) => item.media_type === 'tv' && (item.vote_average || 0) >= 6)
  ...
  if (contentTypeFilter === 'all' || contentTypeFilter === 'movie') {
    ...
    requests.push(fetchFromDiscover('movie', movieParams, 15, 600))
  } else {
  ...
  if (contentTypeFilter === 'all' || contentTypeFilter === 'tv') {
    ...
    requests.push(fetchFromDiscover('tv', tvParams, 15, 600))
  } else {
    requests.push(Promise.resolve([]))
  }
...
async function fetchSingleGenre(contentType, genreId, minVoteAverage) {
  ...
  const primary = await fetchFromDiscover(
    contentType,
    {
...
async function fetchBroadGenreFallback(contentType, genreIds, minVoteAverage) {
  const fallback = await fetchFromDiscover(
    contentType,
    {
      with_genres: genreIds.join('|'),
      sort_by: 'popularity.desc',
      'vote_count.gte': contentType === 'movie' ? 100 : 40,
      'vote_average.gte': Math.max(6, minVoteAverage),
    },
    15,
    1800
  )
...
export async function fetchRecommendations(searchParams) {
  ...
  const [movies, series] = await Promise.all([
    fetchBalancedContent('movie', movieGenres, 250, minVoteAverage),
    fetchBalancedContent('tv', seriesGenres, 250, minVoteAverage),
  ])
...
export async function fetchSimilar(id, contentType) {
  const allResults = []
  for (let page = 1; page <= 15; page++) {
    const data = await tmdbFetch(`/${contentType}/${id}/similar`, { page }, { revalidate: 1800 })
    const results = data?.results ?? []
    if (!results.length) break
    allResults.push(...results)
  }
  
  const uniqueItems = Array.from(new Map(allResults.map(item => [item.id, item])).values())
  return uniqueItems
    .filter(item => (item.vote_average || 0) >= 6)
    .map(item => formatContent(item, contentType))
    .filter(Boolean)
}

export async function fetchCredits(id, contentType) {
  const data = await tmdbFetch(`/${contentType}/${id}/credits`, {}, { revalidate: 1800 })
  return data?.cast?.slice(0, 15) || []
}

export async function fetchExhibitionGenres() {
  const featuredGenres = [
    { id: '18', type: 'movie', name: 'DRAMA', title: 'Emotional /<br/>Journeys', label: 'DRAMA CLASSICS' },
    { id: '878', type: 'movie', name: 'SCI-FI', title: 'Future /<br/>Worlds', label: 'SCI-FI VISIONS' },
    { id: '27', type: 'movie', name: 'HORROR', title: 'Midnight /<br/>Terrors', label: 'HORROR COLLECTION' },
    { id: '35', type: 'movie', name: 'COMEDY', title: 'Light /<br/>Laughs', label: 'COMEDY SPECIALS' },
    { id: '53', type: 'movie', name: 'THRILLER', title: 'Nerve /<br/>Wrackers', label: 'THRILLER PICKS' }
  ]
  
  const results = await Promise.all(featuredGenres.map(async (g) => {
    const voteCount = g.type === 'movie' ? 180 : 80;
    const items = await fetchFromDiscover(g.type, {
      with_genres: String(g.id),
      sort_by: 'popularity.desc',
      'vote_count.gte': voteCount,
      'vote_average.gte': 7.0,
    }, 15, 1800)
    const validItem = items.find(i => i.poster_path) || items[0]
    return {
      ...g,
      count: items.length || 0,
      posterUrl: validItem?.poster_path ? `${POSTER_BASE_URL}${validItem.poster_path}` : null
    }
  }))
  return results
}
