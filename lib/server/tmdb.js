...
async function fetchSingleGenre(contentType, genreId, languageCodes, voteCount, minVoteAverage, itemLimit) {
  const results = []

  for (let page = 1; page <= 3 && results.length < itemLimit; page += 1) {
  ...
  return results.slice(0, itemLimit)
}
...
async function fetchBalancedContent(contentType, genreIds, totalItemsTarget, minVoteAverage) {
  ...
  const itemsPerRegion = Math.max(1, Math.floor(totalItemsTarget / 2))
  const itemsPerGenreEn = Math.max(1, Math.floor(itemsPerRegion / genreIds.length))
  const itemsPerGenreIn = Math.max(1, Math.floor(itemsPerRegion / genreIds.length))
  const voteCountEn = minVoteAverage < 8 ? 500 : 150
  const voteCountIn = 50

  const englishTasks = genreIds.map((genreId) =>
    fetchSingleGenre(contentType, genreId, 'en', voteCountEn, minVoteAverage, itemsPerGenreEn)
  )
  const indianTasks = genreIds.map((genreId) =>
    fetchSingleGenre(contentType, genreId, 'hi|kn|te|ta|ml', voteCountIn, minVoteAverage, itemsPerGenreIn)
  )

  const [englishResults, indianResults] = await Promise.all([
    Promise.all(englishTasks),
    Promise.all(indianTasks),
  ])

  const deduped = []
  const seenIds = new Set()

  for (const item of [...englishResults.flat(), ...indianResults.flat()]) {
    if (seenIds.has(item.id)) {
      continue
    }

    const formattedItem = formatContent(item, contentType)
    if (!formattedItem) {
      continue
    }

    seenIds.add(item.id)
    deduped.push(formattedItem)
  }
...
export async function fetchRecommendations(searchParams) {
  ...
  const seriesGenres = genreOverrideIds.length
    ...
    : emotionGenres.tv
  const minVoteAverage = Number.isFinite(minRatingOverride) ? minRatingOverride : 7

  const [movies, series] = await Promise.all([
    fetchBalancedContent('movie', movieGenres, 200, minVoteAverage),
    fetchBalancedContent('tv', seriesGenres, 200, minVoteAverage),
  ])
