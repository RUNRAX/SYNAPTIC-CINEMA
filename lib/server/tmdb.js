...
export async function fetchHomeContent() {
  ...
  const [trendingMovies, trendingSeries, featuredMovies, popularSeries] = await Promise.all([
    fetchFromDiscover('movie', trendingParams, 3),
    fetchFromDiscover('tv', trendingParams, 2),
    fetchFromDiscover('movie', featuredParams, 3),
    fetchFromDiscover('tv', popularSeriesParams, 3),
  ])
...
export async function searchTmdb(searchParams) {
  ...
  if (contentTypeFilter === 'all' || contentTypeFilter === 'movie') {
    ...
    requests.push(fetchFromDiscover('movie', movieParams, 2, 600))
  } else {
  ...
  if (contentTypeFilter === 'all' || contentTypeFilter === 'tv') {
    ...
    requests.push(fetchFromDiscover('tv', tvParams, 2, 600))
  } else {
    requests.push(Promise.resolve([]))
  }
...
export async function fetchRecommendations(searchParams) {
  ...
  const [movies, series] = await Promise.all([
    fetchBalancedContent('movie', movieGenres, 60, minVoteAverage),
    fetchBalancedContent('tv', seriesGenres, 40, minVoteAverage),
  ])
...
export async function fetchSimilar(id, contentType) {
  const data = await tmdbFetch(`/${contentType}/${id}/similar`, {}, { revalidate: 1800 })
  return (data?.results ?? [])
    .map(item => formatContent(item, contentType))
    .filter(Boolean)
}
